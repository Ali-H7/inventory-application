...
const genreQuery = `SELECT genre_name FROM genre
WHERE UPPER(genre_name) = UPPER($1);`;
async function getOneGenre(genre) {
  const { rows } = await pool.query(genreQuery, [genre]);
  return rows[0];
}
